...
# TMC/MMC   - version x
#           - refined 'get_freeos_kyc' to return a compatible format with the .csv file for 'kyc_level'
#           - changed the default values returned by 'get_freeos_kyc' to empty values
#           - added the configurable fields 'endpoint', 'freeos_contract', and 'sample_size
...
def get_users_table_data(account):
    ...
    if len(json_data['rows']):
        ...
        user_last_issuance = user_record['last_claim']
    else:
        user_stake = '0.000000'
        user_account_type = 0
...
def get_freeos_kyc(account):
    # changed the command to include the configurable field 'endpoint'
    # (original command was f'cleos -u https://proton.greymass.com get table eosio.proton eosio.proton usersinfo --lower {account} --upper {account}')
...
def getusers():
    ...
    # (original command was f"cleos -u https://proton.greymass.com get scope freeosclaim -t users -l 10")
    ...
    with open(f"{freeos_contract}_users_{unix_time}.csv", "w") as g:
